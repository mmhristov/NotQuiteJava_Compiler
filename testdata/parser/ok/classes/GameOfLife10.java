	...
	int calc(Board pred) {
		...
		while (y < height) {
			...
			while (x < width) {
				...
				count = pred.at(x - 1, y - 1) + pred.at(x, y - 1)
						+ pred.at(x + 1, y - 1) + pred.at(x - 1, y)
						+ pred.at(x + 1, y) + pred.at(x - 1, y + 1)
						+ pred.at(x, y + 1) + pred.at(x + 1, y + 1);
...
class Game {
	...
	int run() {
		...
		while (i < n) {
			...
			printInt(hash.hash(bold.board())); // show old board
